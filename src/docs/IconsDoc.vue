...
const setupSnippets: Record<SetKey, { install: string[]; wire: string[] }> = {
    ...
    material: {
        install: [
            ...
            '# Install the per-icon package so you only bundle what you use:',
            'npm install @iconify/vue @iconify-icons/material-symbols',
        ],
        wire: [
            "// main.ts",
            "import { createApp } from 'vue';",
            ...
            "",
            "// Per-icon imports — each file is ~200 bytes, tree-shaken on build",
            ...
            "// …add only the icons you actually reference",
            "",
            ...
            "app.mount('#app');",
            "",
            "// Alternative — bundle the entire set in one line if size isn't a concern:",
            "//   import data from '@iconify-json/material-symbols/icons.json';",
            "//   import { addCollection } from '@iconify/vue';",
            "//   addCollection(data); // adds ~3,000 icons / ~2MB gzipped",
        ],
    },
    solar: {
        install: [
            '# Solar — Iconify-only, same per-icon pattern as Material Symbols',
            'npm install @iconify/vue @iconify-icons/solar',
        ],
        wire: [
            "// main.ts",
            "import { createApp } from 'vue';",
            ...
            "// …browse names at icon-sets.iconify.design/solar",
            "",
